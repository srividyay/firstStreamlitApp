...
BRANCH = st.secrets.get("BRANCH", "main")  # default branch to push to

# Optional Git push config (set in .streamlit/secrets.toml for CI/Cloud)
GIT_REMOTE = st.secrets.get("GIT_REMOTE")  # e.g., "https://github.com/you/your-repo.git"
GIT_PAT = st.secrets.get("GIT_PAT")        # GitHub PAT (scoped minimally)
...
def git_commit_file(repo_root: Path, path_to_add: Path):
    ...
    if isinstance(commit_res, subprocess.CalledProcessError):
        # Possibly nothing to commit (duplicate content), show info
